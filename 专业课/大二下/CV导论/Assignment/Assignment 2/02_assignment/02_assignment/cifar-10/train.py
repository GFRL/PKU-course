...
def MyCELoss(pred, gt):
    ...
    # loss11=loss1(pred,gt)
    # # 以上是调用pytorch自带的交叉熵损失函数
    # 以下是自己实现的交叉熵损失函数
    # 先进行softmax
    pred=torch.exp(pred)
    ...
    pred[pred==0]=1e-10
    pred[pred==1]=1-1e-10
    index=torch.arange(gt.shape[0])
    loss=-torch.log(pred[index,gt])
...
def train(epoch, model, optimizer, criterion, train_loader, writer):
    ...
    for imgs, labels in tqdm.tqdm(train_loader):
        ...
        if iteration % 50 == 0:
            #pass 
            # ----------TODO------------
            writer.add_scalar('Train Loss', {lr_rate_str:losses.avg}, iteration)
            writer.add_scalar('Train Acc@1', {lr_rate_str:top1.avg}, iteration)
            writer.add_scalar('Train Acc@5', {lr_rate_str:top5.avg}, iteration)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--total_epoch', '-t', type=int, default=5, help="total epoch number for training")
