...
//辅助函数
//要加volatile 不然有个点会炸。
volatile int fg_exit;     
//前台作业是否完成 ,在需要的地方置为0，然后前台好了改成1
/*
...
/* 重定向输入输出 */
void reIO(const struct cmdline_tokens tok, int* stdin_, int* stdout_){
    if (tok.infile != NULL){
        int fd = open(tok.infile, O_RDONLY, 0);
        if (fd < 0){
            sio_puts("Open IN-file error!");
            return;
        }
        *stdin_ = dup(0);
        dup2(fd, 0);
    ...
    if (tok.outfile != NULL){
        int fd = open(tok.outfile, O_WRONLY | O_CREAT | O_TRUNC, 0);
        if (fd < 0)
        {
            sio_puts("Open OUT-file error!");
            return;
        }
        *stdout_ = dup(1);
        dup2(fd, 1);
...
void solve_BG(const struct cmdline_tokens tok){
    if (tok.argv[1] == NULL) {
                sio_put("bg command need PID or jobid\n");
            }
            else {
                char* tmp_str = tok.argv[1];
                int target = 0;
                if (tmp_str[0] == '%') {//job id
                    target = atoi(tmp_str + 1);
                    struct job_t* job_ = getjobjid(job_list, target);
                    if (!job_ || job_->state == UNDEF) {
                        sio_puts("CANNOT FIND the JOB!");
                    }
                    else {
                        if(job_->state==ST){
                        kill(-job_->pid, SIGCONT);
                        job_->state = BG;
                        }
                    sio_put("[%d] (%d) ", target, job_->pid);
                    sio_puts(job_->cmdline);
                    sio_puts("\n");
                    }
                }
                else if (tmp_str[0] <= '9'&&tmp_str[0]>='0') {//job id
                    target = atoi(tmp_str);
                    struct job_t* job_ = getjobjid(job_list, target);
                    if (!job_ || job_->state == UNDEF) {
                        sio_puts("CANNOT FIND the JOB!");
                    ...
                    sio_put("BG commmad argument not job or pid");
                }
            }
}
void solve_FG(const struct cmdline_tokens tok){
    ...
    if (tok.argv[1] == NULL) {
                ...
                if (tmp_str[0] == '%') {//job id
                    ...
                    sio_put("FG commmad argument not job or pid");
                }
            }
}
void solve_KILL(const struct cmdline_tokens tok){
...
eval(char* cmdline)
{
    ...
    //重定向
    //保存原来的stdin,stdout
    int stdin_ = -1, stdout_ = -1;
    reIO(tok,&stdin_,&stdout_);
    //处理一下内建函数
    ...
    { 
        ...
        if (tok.builtins == BUILTIN_QUIT) {
            ...
            //solve_KILL(tok);
            if (tok.argv[1] == NULL) {
                sio_put("KILL command need PID or jobid\n");
            }
            else {
                char* tmp_str = tok.argv[1];
                int target = 0;
                if (tmp_str[0] == '%') {//job id
                    ...
                    if (!job_ || job_->state == UNDEF) {
                        ...
                        if(sign)kill(-job_->pid, SIGTERM);
                        else kill(job_->pid, SIGTERM); 
                    }
                }
                else if (tmp_str[0] <= '9'&&tmp_str[0]>='0'){
                    target = atoi(tmp_str);
                    int sign=0;
                    if(target<0){
                        sign=1;target*=-1;
                    ...
                    sio_put("KILL commmad argument not job or pid");
                }
            }
        }
    ...
    //复原
    if (stdin_ > 0)dup2(stdin_, 0), close(stdin_);
    if (stdout_ > 0)dup2(stdout_, 1), close(stdout_);
    return;
}
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED)) > 0)
     {
        ...
        if (pid == fgpid(job_list))fg_exit = 1;//改变前台信息
        if (WIFEXITED(status))deletejob(job_list, pid);//正常
        else if (WIFSIGNALED(status)) {//信号终止  
            sio_put("Job [%d] (%d) terminated by signal %d\n", pid2jid(pid), pid, WTERMSIG(status));
            deletejob(job_list, pid);
        }
        else if (WIFSTOPPED(status)){//信号暂停
            struct job_t* job_ = getjobpid(job_list, pid);
            job_->state = ST;
            sio_put("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), pid, WSTOPSIG(status));
        }
        else if (WIFCONTINUED(status)){//信号重新启动
            struct job_t* job_ = getjobpid(job_list, pid);
            if(job_->state==ST)
                job_->state = BG;
        }
        Sigprocmask(SIG_SETMASK, &prev, NULL);//恢复信号
     }
    errno = pre_errno;
    return;
}
